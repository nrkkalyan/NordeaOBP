...
import org.lwapp.obp.nordea.response.ok.GetAccountTransactionsResponse;
import org.lwapp.provider.MyObjectMapper;

public class GetAccountTransactionsJerseyExample {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        WebTarget accountsWebTarget = webTarget.path("accounts").path("FI6593857450293470-EUR").path("transactions");
        ...
        if (response.getStatus() == 200) {
            GetAccountTransactionsResponse getAccountsResponse = response.readEntity(GetAccountTransactionsResponse.class);
